...
class Kill extends Message {
    ...
    public function __construct() {
        parent::__construct('/^"(?P<user_name>.+)[<](?P<user_id>\d+)[>][<](?P<steam_id>.*)[>][<](?P<user_team>CT|TERRORIST|Unassigned|Spectator)[>]" killed "(?P<killed_user_name>.+)[<](?P<killed_user_id>\d+)[>][<](?P<killed_steam_id>.*)[>][<](?P<killed_user_team>CT|TERRORIST|Unassigned|Spectator)[>]" with "(?P<weapon>[a-zA-Z0-9]+)"(?P<headshot>.*)/');
    }
    ...
    public function process() {
        ...
        $o->setHeadshot(trim($this->datas['headshot']) == "(headshot)");
