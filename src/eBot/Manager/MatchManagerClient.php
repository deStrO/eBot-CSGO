    ...
    public function sendPub() {
        foreach ($this->matchs as $k => $match) {
            if ($match->getStatus() == Match::STATUS_END_MATCH) {
                ...
                $socket->sendTo("removeMatch " . $match->getMatchId(), \eBot\Config\Config::getInstance()->getBot_ip(), \eBot\Application\ApplicationClient::getInstance()->getPortMain());
                unset($this->matchs[$k]);
        ...
        while ($req = mysqli_fetch_assoc($sql)) {
            if (!@$this->matchs[$req['server_ip']]) {
                try {
                    ...
                    mysqli_query("UPDATE `matchs` SET enable=0 WHERE id = '" . $req['match_id'] . "'") or die(mysqli_error());
                } catch (\Exception $ex) {
